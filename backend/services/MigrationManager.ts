...
class MigrationManager extends DatabaseManager {
    ...
    async migrate(firebirdConnectionId: any, postgresConfig: any) {
        ...
        try {
            ...
            if (!tablesResult.success || !tablesResult.data.length) {
                ...
                throw new Error('No se encontraron tablas');
            }
            ...
            for (let table of tablesResult.data) {
                    await this.addForeignKeys(pg, firebirdConnectionId, table.TABLE_NAME);
            }
 ...
 async addForeignKeys(pg: { query: (arg0: string) => any; }, connectionId: any, tableName: string) {
    ...
    const result = await this.getTableConstraints(connectionId, tableName);
    
    if (result.success && result.data) {

        const foreignKeys = result.data.filter((constraint: { CONSTRAINT_TYPE: string; }) => 
        ...
        for (const fk of foreignKeys) {
            const fkSQL = `
                ...
                ADD CONSTRAINT fk_${tableName.toLowerCase()}_${fk.COLUMN_NAMES.toLowerCase()}
                FOREIGN KEY (${fk.COLUMN_NAMES.toLowerCase()}) 
                REFERENCES ${fk.REFERENCED_TABLE_NAME.toLowerCase()}(${fk.REFERENCED_COLUMN_NAMES.toLowerCase()})
            `;
